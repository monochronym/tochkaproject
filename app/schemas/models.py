...
class MarketOrderBody(Base):
    order_id: Mapped[UUID] = mapped_column(ForeignKey("marketorders.id"), primary_key=True)
...
class LimitOrderBody(Base):
    order_id: Mapped[UUID] = mapped_column(ForeignKey("limitorders.id"), primary_key=True)
    direction: Mapped[Direction] = mapped_column(Enum(Direction), nullable=False)
    ticker: Mapped[str] = mapped_column(ForeignKey("tickers.ticker"), nullable=False)
    qty: Mapped[int]
    price: Mapped[int]
